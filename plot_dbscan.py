...
data = np.genfromtxt("catalogo.dat",unpack=True)

indx = data[2] < 0.05
N = indx.sum()

X = np.zeros((N,3))
#X[:,0] = data[2][indx]*np.cos(data[0][indx])*np.cos(data[1][indx])
#X[:,1] = data[2][indx]*np.sin(data[0][indx])*np.cos(data[1][indx])
#X[:,2] = data[2][indx]*np.sin(data[1][indx])
